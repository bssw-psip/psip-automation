...
public class RDFUtils {
    ...
    public static String writeRDFData(String format, Model model){
        logger.info("[DATA] Writing data outstream format="+format);
        String output;
        ...
        logger.info("[DATA] Writing data outstream1="+rdfFormat);

        Rio.write(model, outstream, rdfFormat);
        logger.info("[DATA] Writing data outstream2");

        output = new String(outstream.toByteArray(), StandardCharsets.UTF_8);
        logger.info("[DATA] Writing data outstream3");
