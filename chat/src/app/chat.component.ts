...
export class ChatComponent implements OnInit, AfterViewChecked {
  ...
  async sendWithDelay(param: any, delayType: string) {
    ...
    if (temp_block.block_subtype == "text") {
      ...
      litem = $('<li/>').addClass('chat-li coney').appendTo($('#chat-ul-list')).append($('<p/>').html(temp_block.text).addClass('coney-msg message'));
      ...
      litem = $('<li/>').addClass('chat-li coney').appendTo($('#chat-ul-list')).append(($('<div/>').addClass('row m-0')).append($('<a/>').attr('href', temp_block.link).attr('target', '_blank').attr('rel', 'noopener').html(temp_block.text).addClass('coney-msg link message')));
